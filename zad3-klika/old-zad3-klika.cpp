...
struct Graf {
	...
	int degree(int index) {     //metoda zwracajaca stopien wierzcholka o podanym indeksie
		int deg = 0;
		for (int i = 0; i < lista_krawedzi.size(); i++) {
			if (lista_krawedzi[i][0] == index || lista_krawedzi[i][1] == index)
				deg++;      //zliczane jest kazde wystapienie podanego indeksu
		}
		return deg;
	}

	std::vector<int> neighbours(int index) {     //metoda zwraca wektor indeksow wierzcholkow ktore sa polaczone krawedzia z podanym wierzcholkiem
		std::vector<int> n;
		for (int i = 0; i < lista_krawedzi.size(); i++) {
	...
	std::vector<std::vector<int>>  szukanie_kliki(int rozmiar) {
		...
		for (int i = 0; i < lista_krawedzi.size(); ++i) {
			for (int x = 0; x < 2; ++x) {
				...
				if (degree(lista_krawedzi[i][x]) >= rozmiar - 1) {

					potential_clique.push_back(lista_krawedzi[i][x]);    //potential_clique bedzie zawierac indeks sprawdzanego wierzcholka i indeksy jego sasiadow
					temp1 = neighbours(lista_krawedzi[i][x]);
					potential_clique.insert(potential_clique.end(), temp1.begin(), temp1.end());
					temp1.clear();
					sort(potential_clique.begin(), potential_clique.end());

					for (int j = 0; j < potential_clique.size(); j++) {      //sprawdzamy kazdy element potencjalnej kliki
						temp1.push_back(potential_clique[j]);
						temp2 = neighbours(potential_clique[j]);
						temp1.insert(temp1.end(), temp2.begin(), temp2.end());
						temp2.clear();
						sort(temp1.begin(), temp1.end());
						std::vector<int>::iterator it;
						for (int k = 0; k < potential_clique.size(); k++) {				
							if (find(temp1.begin(), temp1.end(), potential_clique[k]) == temp1.end()) {    //jesli jakiegos elementu z potential_clique nie ma na temp1, to jest on usuwany z potential_clique
					...
					if (potential_clique.size() > rozmiar - 1) {   //do cliques beda wpisane tylko kliki rozmiar-1w
						bool already_in = false;
						for (int n = 0; n < cliques.size(); n++) {    //sprawdzamy czy klika zapisana w potential_clique nie jest juz zapisana w cliques
							if (equal(cliques[n].begin(), cliques[n].end(), potential_clique.begin()))
								already_in = true;
						}
						if (!already_in)         //jesli w cliques nie znajduje sie juz potential_clique
							cliques.push_back(potential_clique);    //zapis znalezionej kliki do cliques
					}
...
int main() {
	...
	std::cout << "Podaj nazwe instancji: ";
	std::cin >> plik;

	std::cout << "Podaj dlugosc podciagu: (w zakresie 4 - 9) ";
	std::cin >> dlugosc;
	...
	std::cout << "instancja: " << plik << "  dlugosc podsekwencji: " << dlugosc << "  prog wiarygodnosci: " << prog_wiarygodnosci << endl;
	...
	if (kliki.empty()) {
		...
		cout << "We wszystkich sekwencjach wystepuje podciag:" << endl;
		for (int i = 0; i < kliki.size(); i++) {
			index = kliki[i][0];
			cout << graf.wierzcholki[index].podsekwencja << endl;
			for (int j = 0; j < kliki[i].size(); j++) {
				index = kliki[i][j];
				cout << "\tSekwencja " << graf.wierzcholki[index].sekwencja_wejsciowa.id << "  Pozycja: " << graf.wierzcholki[index].pozycja << endl;
			}
		}
	}
	return 0;
