...
struct Graf {
	...
	std::vector<std::vector<int>>  szukanie_kliki(int rozmiar) {
		...
		std::vector<std::vector<int>> kliki;
		std::vector<int> potencjalna_klika, temp1, temp2;
		...
		for (int i = 0; i < lista_krawedzi.size(); ++i) {
			for (int x = 0; x < 2; ++x) {
				...
				if (stopien(lista_krawedzi[i][x]) >= rozmiar - 1) {
					...
					potencjalna_klika.push_back(lista_krawedzi[i][x]);
					temp1 = sasiedzi(lista_krawedzi[i][x]);
					potencjalna_klika.insert(potencjalna_klika.end(), temp1.begin(), temp1.end());
					temp1.clear();
					sort(potencjalna_klika.begin(), potencjalna_klika.end());

					for (int j = 0; j < potencjalna_klika.size(); j++) {
						temp1.push_back(potencjalna_klika[j]);
						temp2 = sasiedzi(potencjalna_klika[j]);
						temp1.insert(temp1.end(), temp2.begin(), temp2.end());
						temp2.clear();
						sort(temp1.begin(), temp1.end());
						std::vector<int>::iterator it;
						for (int k = 0; k < potencjalna_klika.size(); k++) {
							if (find(temp1.begin(), temp1.end(), potencjalna_klika[k]) == temp1.end()) {								
								potencjalna_klika.erase(find(potencjalna_klika.begin(), potencjalna_klika.end(), potencjalna_klika[k]));
								--k;
							}
						}
						temp1.clear();
					}
					if (potencjalna_klika.size() > rozmiar - 1) {
						bool already_in = false;
						for (int n = 0; n < kliki.size(); n++) {
							if (equal(kliki[n].begin(), kliki[n].end(), potencjalna_klika.begin()))
								already_in = true;
						}
						if (!already_in)
							kliki.push_back(potencjalna_klika);
					}
					potencjalna_klika.clear();
				}
			}
		}
		if (kliki.empty()) {
